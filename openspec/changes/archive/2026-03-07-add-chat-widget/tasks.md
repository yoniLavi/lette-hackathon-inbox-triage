...
- [x] 3.4 Add a per-request timeout to `receive_response()` (90s resetting deadline via `asyncio.timeout_at` + `reschedule`) with automatic session teardown and error SSE event on timeout
...
- [x] 4.2 Test: sending a message while agent is busy returns 409 (covered by `test_409_when_busy` in `tests/test_chat_e2e.sh`)
