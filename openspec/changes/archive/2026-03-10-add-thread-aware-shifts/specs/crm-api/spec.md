...
The CRM API SHALL expose REST endpoints for managing Properties, Contacts, Emails, Tasks, Cases, and Notes with consistent JSON responses, filtering, ordering, and pagination.
...
The CRM API SHALL support a `?include=` query parameter on GET-by-ID endpoints that returns related records nested in the response, reducing round-trips for composite data needs.
...
- **THEN** the API returns `404` with `{"detail": "No unread threads"}`
...
- **WHEN** a client sends `PATCH /api/emails/bulk` with `{"ids": [], "updates": {"is_read": true}}`
- **THEN** the API returns `{"updated": 0}`
