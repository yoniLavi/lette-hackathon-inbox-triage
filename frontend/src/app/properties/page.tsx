...
import { getAccounts } from "@/lib/espo";
import type { CrmAccount } from "@/lib/espo";

export default function PropertiesView() {
    const [accounts, setAccounts] = useState<CrmAccount[]>([]);

    useEffect(() => {
        getAccounts().catch(() => []).then(setAccounts);
    }, []);
            ...
            <main className="flex-1 w-full max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {accounts.map(account => (
                        <Card key={account.id} className="p-6 hover:shadow-md transition-shadow relative overflow-hidden group bg-[#F2F2EC] border-transparent">
                            ...
                            <div>
                                ...
                                {account.description && (
                                    <p className="text-sm text-[#0F1016]/60 mt-2 font-sans whitespace-pre-line">{account.description}</p>
                                )}
                            </div>
                        </Card>
                    ))}
